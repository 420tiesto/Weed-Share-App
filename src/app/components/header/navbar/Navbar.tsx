            ...
            <nav className="flex items-center h-full justify-between">
                ...
                <div className="flex gap-6  items-center flex-1 justify-end ">
                    <div className="flex-grow ">
                        <Input leftIcon={<SearchIcon className="h-5 w-5" />} placeholder="Search music, projects, and accounts" className='w-full' />
                    </div>
                    <div className="inline-flex pl-8 gap-6">
                        {NAVBAR_LINKS.map(({ name, href }: NavlinkType) => (
                         ...
                         {AUTHENTICATED_NAV_LINKS.map(({ name, href }: NavlinkType) => (
                            <AuthenticatedNavlink authState={authenticatedState} href={href}>{name}</AuthenticatedNavlink>
                        ))} 
                    </div>
                    {authenticatedState ? <UserMenu /> : <UserCircleIcon onClick={handleNavigate} className='h-8 w-8 hover:bg-white/10  rounded-lg cursor-pointer'/> }
                </div>
            </nav>
        </header>
    );
